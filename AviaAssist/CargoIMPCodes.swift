...
//    ]
    var codesAndDefinitions: [(code: String, definition: String)] = []
    ...
    func fetchCargoIMPCodes() {
        databaseRef.child("cargoIMPCodes").observeSingleEvent(of: .value) { snapshot in
            print("Snapshot: \(snapshot)") // Debug print for the snapshot
            
            if let value = snapshot.value as? [String: String] {
                print("Fetched Data: \(value)") // Debug print
                self.codesAndDefinitions = value.map { (key, val) in (code: key, definition: val) }
                self.codesAndDefinitions.sort { $0.code < $1.code }
                
                DispatchQueue.main.async {
                    self.codePickerView.reloadAllComponents()
                }
            } else {
                print("Error: Unable to fetch data or data is empty.") // Debug print
            }
        } withCancel: { error in
            print("Firebase Error: \(error.localizedDescription)") // Debug print for Firebase errors
        }
    }
